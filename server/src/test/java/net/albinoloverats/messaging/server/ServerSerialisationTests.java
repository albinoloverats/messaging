...
class ServerSerialisationTests
{
	...
	void verify_broadcast_lifecycle()
	{
		...
		val messageType = MessageSerialiser.findEventType(serialised);
		serialised.rewind();
		assertEquals(PeerRelay.class.getName(), messageType);

		val peerTriple = MessageSerialiser.separateMessage(serialised);
		val peerBuffer = peerTriple.getLeft();
		PeerRelay deserialised = MessageSerialiser.deserialiseMessage(peerBuffer);
		assertEquals(id, deserialised.id());

		val payload = deserialised.event();
		val messageTriple = MessageSerialiser.separateMessage(ByteBuffer.wrap(payload));
		event = messageTriple.getLeft();
		val recovered = MessageSerialiser.deserialiseMessage(event);
